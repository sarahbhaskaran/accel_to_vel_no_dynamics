...
// Generated 06-May-2022 00:31:28
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"accel_to_vel_no_dynamics\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(accel_to_vel_no_dynamics_M, (NULL));
        accel_to_vel_no_dynamics_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(accel_to_vel_no_dynamics_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(accel_to_vel_no_dynamics_M)
                == (NULL)) || rtmGetStopRequested(accel_to_vel_no_dynamics_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        accel_to_vel_no_dynamics_terminate();
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(accel_to_vel_no_dynamics_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
